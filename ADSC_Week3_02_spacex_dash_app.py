...
def get_pie_chart(site_dropdown):
    if site_dropdown == 'ALL':
        ...
        figure = px.pie(filtered_df, values='class', names='Launch Site', title='Total Launches for All Sites')
        return figure
    else:
        selected_df = spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
        figure = px.pie(selected_df, names='class', title='Total Launch for a Selected Site')
        return figure
# TASK 4:
...
def get_scatter_chart(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        ...
        selected_df = spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
        filtered_df = selected_df.loc[(selected_df['Payload Mass (kg)'] >= payload_slider[0]) & (selected_df['Payload Mass (kg)'] <= payload_slider[1])]
        figure = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return figure
# Run the app
